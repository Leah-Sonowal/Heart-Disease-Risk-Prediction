...
def banner():
    ...
    print("  heart disease.\n")
    print(clr("  ⚠  This is NOT a medical diagnosis.", YELLOW))
...
def build_feature_row(raw, train_columns, scaler, num_cols):
    ...
    # IMPORTANT: use the SAME scaler fitted on training data, not a new one.
...
def load_and_train(csv_path="final_unified_dataset.csv"):
    ...
    # Re-fit a scaler on the full dataset so we can transform user input
    # the same way the training data was transformed.
    # NOTE: the CSV already contains z-scored values; we refit on those
    # scaled values so new user input can be standardised to the same scale.
